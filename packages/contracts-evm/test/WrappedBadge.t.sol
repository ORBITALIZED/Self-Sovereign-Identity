...
contract WrappedBadgeTest is Test {
    ...
    function test_lock_emits_event() public {
        vm.prank(issuer);
        uint256 tid = sbt.issueCredential(holder, SCHEMA, "QmCID", uint64(block.timestamp + 30 days));
        ...
        emit BadgeLocked(holder, SCHEMA, tid, 1_700_000_000 /*stellar pubnet chain id*/, stellarHash);
