...
|---|---|
| `credential.circom` | User holds a valid credential issued by one of an allow-listed set of issuers, *without revealing which credential* |
| `age_verification.circom` | User is older than `minAge`, *without revealing their date of birth* |
