  ...
  const list = [
    {
      ...
      title: "Universidad de los Andes — BSc Computer Science",
      year: 2022,
      status: "valid",
    },
    {
      icon: <Briefcase className="w-4 h-4" />,
      title: "Acme — Senior Engineer Letter",
      year: 2024,
      status: "valid",
    },
    {
      icon: <HeartPulse className="w-4 h-4" />,
      title: "Hospital San Ignacio — Vaccination record",
      year: 2025,
      status: "valid",
    },
      ...
      <div className="lg:col-span-2 space-y-3">
        {list.map((c) => (
          <Card key={c.title} className="p-4 flex items-center gap-4">
            ...
            <span
              className={`text-xs px-2 py-1 rounded-full ${c.status === "valid" ? "bg-emerald-700/30 text-emerald-300" : "bg-brand-700/30 text-brand-300"}`}
            >
              {c.status}
            </span>
            {c.status === "zkProof" && (
