...
export default function CreateIdentity() {
  ...
  async function enrollBiometric() {
    ...
    try {
      ...
      setCommit("0x" + Array.from({ length: 32 }, () => Math.floor(Math.random() * 256).toString(16).padStart(2, "0")).join(""));
      ...
      <div className="px-6 py-8 min-h-[280px]">
        {step === 0 && (
          <div className="text-center space-y-4">
            ...
            <p className="text-sm text-slate-400 max-w-md mx-auto">
              Your device's secure enclave will hash the biometric locally; only the commitment ever leaves the device.
            </p>
            <Button onClick={enrollBiometric} loading={busy}>Capture</Button>
            {commit && <code className="block text-xs text-slate-300">commit: {commit.slice(0, 22)}…</code>}
          </div>
        )}
        ...
        {step === 1 && (
          <div className="space-y-4">
            ...
            <p className="text-xs text-slate-500">Stored on IPFS, encrypted with a wallet-derived AES-GCM key.</p>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="space-y-4">
            ...
            <input className="w-full surface-card px-3 py-2 text-sm" placeholder="Guardian Stellar pubkey (G…)" />
            <input className="w-full surface-card px-3 py-2 text-sm" placeholder="Another guardian (optional)" />
        ...
        {step === 3 && (
          <div className="space-y-4">
            ...
            <p className="text-sm text-slate-300">Once submitted, your identity will be created on Stellar Testnet in ~5 seconds.</p>
            <Button onClick={submit} loading={busy}>Submit to Soroban</Button>
          </div>
        )}
      </div>
...
function Stepper({ current }: { current: number }) {
  return (
    <ol className="flex gap-3 mt-3 text-xs">
      {STEPS.map((label, i) => (
        <li key={label} className={`flex items-center gap-1 ${i === current ? "text-brand-500" : "text-slate-500"}`}>
          <span className={`h-1.5 w-6 rounded-full ${i <= current ? "bg-brand-500" : "bg-surface-700"}`} />
          <span>{label}</span>
        </li>
      ))}
    </ol>
